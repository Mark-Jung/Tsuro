    ...
    public class Tournament
    {
        ...
        static void Main(string[] args)
        {
            if(args.Length == 0){
                ...
                IPEndPoint remoteEP = new IPEndPoint(ip, tournament_player.port);
                ...
                //arg[1] color
                //arg[2] portnumber
                if (tournament_player.toPlay == 0)
                {
                    while (true)
                    {
                        tournament_player.ProxyPlay(name, color, writer, reader);
                    }
                }
                else
                {
            ...
            else if(args[0] == "player")
            {
                ...
                IPAddress local = localInfo.AddressList[0];

                Socket sender = new Socket(local.AddressFamily, SocketType.Stream, ProtocolType.Tcp);
                Console.WriteLine("Made another socket for proxy player");
                IPEndPoint remoteEP = new IPEndPoint(ip, tournament_player.port);
                // Connect the socket to the remote endpoint. Catch any errors.  
                sender.Connect(remoteEP);
                Console.WriteLine("Connected to the endpoint, which is: " + remoteEP);
                NetworkStream networkStream = new NetworkStream(sender);
                StreamWriter writer = new StreamWriter(networkStream);
                StreamReader reader = new StreamReader(networkStream);
                if(tournament_player.toPlay == 0){
